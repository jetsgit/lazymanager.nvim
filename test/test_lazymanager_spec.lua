...
describe('LazyManager', function()
  ...
  describe('restore_file_full', function()
    ...
    it('errors if JSON is invalid', function()
      local err = stub(vim.api, 'nvim_err_writeln')
      io.open = function() return { read = function() return invalid_json end, close = function() end } end
      vim.fn.json_decode = function() error('decode error') end
      LazyManager.restore_file_full(test_backup_path)
    ...
    it('cancels if user does not confirm', function()
      io.open = function() return { read = function() return valid_json end, close = function() end } end
      vim.fn.json_decode = function(str) return { ["test-plugin"] = "abc123" } end
      vim.fn.fnamemodify = function(path, _) return 'test-backup.json' end
      local input_called = false
      vim.ui.input = function(opts, cb) input_called = true; cb('n') end
      local printed = false
      _G.print = function(msg) if msg == 'Restore canceled.' then printed = true end end
    ...
    it('restores all plugins if user confirms', function()
      ...
      vim.v.shell_error = 0
      local error_msgs = {}
      vim.api.nvim_err_writeln = function(msg) table.insert(error_msgs, msg) end
      local printed_msgs = {}
      _G.print = function(msg) table.insert(printed_msgs, msg) end
      package.loaded.lazy = {
        plugins = function() return { { name = "test-plugin", dir = plugin_dir, commit = "abc123" } } end
      }
      vim.fn.fnamemodify = function(path, _) return 'test-backup.json' end
      ...
      for _, msg in ipairs(printed_msgs) do
        if msg:match('Restored plugin') then found_restore = true end
        if msg:match('Restart Neovim') then found_restart = true end
      end
